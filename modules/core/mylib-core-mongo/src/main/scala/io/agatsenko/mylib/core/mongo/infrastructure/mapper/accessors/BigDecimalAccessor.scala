...
class BigDecimalAccessor extends FieldAccessor[BigDecimal] {
  override def from(bson: BsonValue): BigDecimal = getValue(bson.asDecimal128())

  override def to(value: BigDecimal): BsonValue = new BsonDecimal128(new Decimal128(value.bigDecimal))

  override def set(doc: BsonDocument, name: String, value: BigDecimal): Unit = doc.put(name, to(value))
