...
class IntAccessor extends FieldAccessor[Int] {
  override def toValue(bson: BsonValue): Int = getValue(bson.asInt32())

  override def toBson(value: Int): BsonValue = new BsonInt32(value)

  override def set(doc: BsonDocument, name: String, value: Int): Unit = doc.put(name, toBson(value))
